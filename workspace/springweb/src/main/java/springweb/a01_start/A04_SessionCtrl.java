...
import springweb.z01_vo.Member011;
...
public class A04_SessionCtrl {
	...
	public Member011 getMember() {
		return new Member011();
	}
	// http://localhost:5080/springweb/login77.do
	...
	public String login88(@ModelAttribute("member") Member011 m, Model d) {
	...
	// http://localhost:5080/springweb/sessionexp01.do
	@RequestMapping("sessionexp01.do")
	public String sessionexp01(HttpServletRequest request) {
		HttpSession session= request.getSession();
		session.setAttribute("m01", new Member011("홍길동","관리자"));
	...
	// http://localhost:5080/springweb/callSession03.do
	@RequestMapping("callSession03.do")
	public String callSession03(HttpServletRequest request) {
		HttpSession session= request.getSession();
		session.setAttribute("prod", new Product("연필",1200,12));
		return "WEB-INF/views/a01_basic/a10_session01.jsp";
	}
