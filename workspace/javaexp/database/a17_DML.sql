...
클라이언트1 데이터를 변경해서 commit하기 전까지는 클라이언트1 화면에서만 변경된 데이터를 보인다.
    rollback을 하면 클라이언트1에게 이전 변경전 데이터로 변경된다.
    다시 변경후, commit하는 순간 클라이언트2, 클라이언트3에게 데이터를 확인했을 때, 변경된 데이터를 볼 수 있다.
