...
# 묵시적 형변환
1. 묵시적인 데이터 타입 변환은 정확한 연산을 위하여 오라크서버 시스템 내부에서 타입을 내부적으로 변환해서 처리해주는 경우를 말한다.
*/
...
SELECT sysdate,
    ...
    to_char(sysdate, 'MM'), -- 월 표현
    to_char(sysdate, 'DD'), -- 일 표현
    to_char(sysdate, 'YYYY/MM/DD') -- 현재 날짜를 YYYY/MM/DD 표현
FROM dual;
-- ex)
SELECT ename 사원명, to_char(hiredate, 'YYYY') 입사년도, to_char(hiredate, 'MM') 입사월, to_char(hiredate, 'DD') 입사일, hiredate
FROM emp
...
/*
    3) to_char(숫자형, '출력형식')
        ...
        - 출력 형식 : $9999(달러형 표기), 9999.99(소숫점 두자리),
                    '9,999,999' 해당 자리수 안에서 첫단위, (콤마) 표시
*/
...
FROM emp;
-- ex)
SELECT ename, sal, to_char(sal+nvl(comm,0), '$9,999,999')
FROM emp;
/*
    4) to_number
...
*/
-- 입사년도 1981 ==> 올해연도로 변경하여 데이터를 출력하자
SELECT to_char(sysdate, 'YYYY') "올해연도"
FROM dual;
SELECT ename, hiredate, to_char(sysdate, 'YYYY') || to_char(hiredate, 'MMDD') "올해입사",
